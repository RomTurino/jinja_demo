...
available_types_of_animals = ["кот", "пёс", "рыбка", "черепаха", "хомяк", "морская свинья", "обычная свинья"]
example_user = lambda : {
...
class Pet(BaseModel):
    ...
    * type - тип животного ("кот", "пёс", "рыбка", "черепаха", "хомяк", "морская свинья", "обычная свинья")
...
app.mount("/static", StaticFiles(directory="module16/static"), name="static")

@app.get("/", tags=["users"])
def get_all_items(request: Request, responce_class=HTMLResponse):
    '''
    Выводит список со всеми пользователями
    '''
    for i in range(len(users)):
        users[i]["pet_url"] = users[i]["pet"]["type"].name.capitalize() + ".png"
